...
N = 300					#numero de passos na varredura de frequencia
PASSO = (FF-freq)/N			#comprimento do passo na varredura de frequencia
TAM = 10000	 			#tamanho da serie em uma certa frequencia
CUT = TAM/2				#corte na serie para diminuir o tamanho do arquivo
...
L = 1.0e-2
R = 100.
V0s = 3.				#amplitude da senoide de entrada
...
##### Método de Runge-Kutta para resolução de eq. diferenciais
def rk4o(k = zeros((5,DIM), Float), x = array([0.,0.,0.]), F = [f,g,h]): #funcao que resolve EDO por Runge Kutta 4a ordem
  for i in range(1,4):
    for j in range(DIM):
      k[i][j] = d*F[j](x[0]+0.5*k[i-1][0],x[1]+0.5*k[i-1][1],x[2]+0.5*k[i-1][2])
  for j in range(DIM):
    k[4][j] = d*F[j](x[0]+k[3][0],x[1]+k[3][1],x[2]+k[3][2])
  for j in range(DIM):
...
info.write('# Corte na serie para diminuir o tamanho do arquivo (CUT): %d\n ' % CUT)
...
for n in range(N):			#n varia a frequencia
	serie = open(diret+'/'+nomeserie + '%d' % n + '.dat','w')
...
##### Runge Kutta de ordem 4
	  ...
	  for i in range(1,4):	#calculo de k1, k2 e k3
	    for j in range(DIM):
	      k[i][j] = d*F[j](x[2][0]+0.5*k[i-1][0],x[2][1]+0.5*k[i-1][1],x[2][2]+0.5*k[i-1][2])
	  for j in range(DIM):	#calculo de k4
	    k[4][j] = d*F[j](x[2][0]+k[3][0],x[2][1]+k[3][1],x[2][2]+k[3][2])

	  for j in range(DIM):	#calculo de x (V, I e theta)
	    dx[j] = (k[1][j]+2.0*k[2][j]+2.0*k[3][j]+k[4][j])/6.
	    x[0][j], x[1][j] = x[1][j], x[2][j] #x[0]=x(anterior,anterior), x[1]=x(anterior), x[2]=x(presente)
	    x[2][j] += dx[j]	#x(presente)

##### Fim Runge Kutta de ordem 4
##### Guardar serie e maximos

   	  if (cont>CUT): serie.write('%.13f\t%.13f\t%.13f\n' % (t, x[2][0], x[2][1]))
	  soma[0] += x[2][0] #soma de V
	  soma[1] += x[2][1] #soma de I
	  if (x[1][0] <= x[2][0])and(x[1][0] <= x[0][0]):   #minimos tensão V (Va>V e Va>Vaa)
	    somamax[0] += x[1][0]
	    nmax[0] += 1
	    bif[0].write('%.13f %.13f\n' % (freq, x[1][0]))
...
##### Media das variaveis
	print('f: %.13f medV: %.13f medI: %.13f' % (freq, soma[0]/N, soma[1]/N))
	med.write('%.13f %.13f %.13f\n' % (freq, soma[0]/N, soma[1]/N))
